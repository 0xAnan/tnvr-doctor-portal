...
export default function CommitteeModal({ isOpen, onClose, onSave, editingCommittee, existingDoctors = [] }) {
  ...
  useEffect(() => {
    if (editingCommittee) {
      ...
      setFormData({
        ...
        _preserveExistingImages: Boolean(editingCommittee._preserveExistingImages)
      });
    } else {
      setFormData({
        title: '',
  ...
  const handleFileUpload = async (e) => {
    if (formData._preserveExistingImages) return;
      ...
      <div className="relative w-full max-w-2xl clean-card rounded-2xl shadow-xl overflow-hidden my-6">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="pt-2 space-y-3 border-t border-slate-200 dark:border-slate-800">
            ...
            {formData._preserveExistingImages && (
              <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 text-xs font-semibold text-amber-800 dark:text-amber-300">
                الصور الأصلية محفوظة في السحابة. يمكن تعديل البيانات الآن، وسيظل قسم الصور دون تغيير حتى عودة الاتصال.
              </div>
            )}

            {!formData._preserveExistingImages && <div className="flex flex-wrap gap-2">
